...
**Current Phase:** Phase 2 — Observer Service

**Overall Progress:** Phase 1 Completed (Project Scaffolded)
...
| LLM provider | Gemini Flash (gemini-1.5-flash) — zero cost, high free-tier limits |
...
- Every `remember()` call: `self_improvement=False`, `dataset_name` set. Must be run in an application-level async background task.
- Every `remember()` call's `data_id` UUID (extracted from the awaited `RememberResult`) must be stored in SQLite log immediately.
...
- External calls only for LLM processing (Gemini Flash). Cognee also makes LLM calls during remember() and improve() — same quota.
...
- Memory Summary Graph is in MVP scope (lightweight, not interactive).
- Visual graph exploration (pan/zoom/navigation) is post-hackathon.
...
Phase 6 — MCP Server                (before Dashboard — core differentiator)
...
- Do not introduce paid LLM APIs. Use Gemini Flash (gemini-1.5-flash) only.
